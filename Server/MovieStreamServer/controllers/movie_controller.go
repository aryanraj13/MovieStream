...
func GetReviewRanking(admin_review string, client *mongo.Client, c *gin.Context) (string, int, error) {
	...
	if geminiAPIKey == "" {
		return "", 0, errors.New("could not read GOOGLE_API_KEY")
	}

	// Initialize Gemini
	llm, err := googleai.New(context.Background(), googleai.WithAPIKey(geminiAPIKey), googleai.WithDefaultModel("gemini-3.6-flash"))
